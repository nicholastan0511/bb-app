...
verseRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // If the verse doesn't exist, create a new one
      if (!existingVerse) {
        existingVerse = new Verse({ ...body, userId: user.id });
        await existingVerse.save();
      }

      // check if the verse is already saved by user
      if (user.savedVerses.includes(existingVerse._id)) {
        return res
          .status(409)
          .json({ error: 'Verse already saved by this user' });
      }

      // save the verse to user's saved verses
